      ...
      <div className="flex items-center justify-between text-sm">
        ...
        {Object.keys(filters).some(key => filters[key as keyof FilterOptions]) && (
          <span className="text-green-600 bg-green-100 px-2 py-1 rounded-sm text-xs">
            {Object.keys(filters).filter(key => filters[key as keyof FilterOptions]).length} Filter
          </span>
        )}
      </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['All', 'Starred', 'Others'].map((option) => (
                <button
                  key={option}
                  onClick={() => handleFilterChange('status', option === 'All' ? '' : option)}
                  className={`px-3 py-1 text-sm rounded-full transition-colors duration-200 ${
                    filters.status === option || (option === 'All' && !filters.status)
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {['All', 'Human', 'Alien'].map((option) => (
                <button
                  key={option}
                  onClick={() => handleFilterChange('species', option === 'All' ? '' : option)}
                  className={`px-3 py-1 text-sm rounded-full transition-colors duration-200 ${
                    filters.species === option || (option === 'All' && !filters.species)
                      ? 'bg-purple-100 text-purple-700 border border-purple-300'
                      : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                  }`}
                >
